...
async function t3_survives_restart() {
  ...
  // kill -9 网关 (python3 youdao_course.py)。run.sh 监督器会在 ~2-4s 内自动重启它(web 不受影响)。
  try {
    execSync(`pkill -9 -f 'youdao_course.py serve'`, { stdio: "ignore" });
  } catch { /* 可能已无进程 */ }
  const ping = async () => { try { await j(`${GW}/api/_debug`); return true; } catch { return false; } };
  // 先等监督器自愈(~10s 窗口); 兜底: 仍未回(无监督环境下单独跑本脚本)则手动拉起一个网关。
  let up = false;
  for (let i = 0; i < 20; i++) { if (await ping()) { up = true; break; } await new Promise((r) => setTimeout(r, 500)); }
  if (!up) {
    execSync(
      `cd /Users/zhb/Documents/julestest && nohup python3 youdao_course.py serve -r req.txt > /tmp/_gw_e2e.log 2>&1 & disown`,
      { stdio: "ignore", shell: "/bin/bash" },
    );
    for (let i = 0; i < 40; i++) { if (await ping()) { up = true; break; } await new Promise((r) => setTimeout(r, 500)); }
  }
...
// 监督模式下 T3 (kill -9 网关) 不再连带杀 web——run.sh 会自动重启网关、web 始终在线。
// 仍把破坏性的 T3 放最后、其余断言 (T1/T2/T4/T5) 在前，保持执行顺序稳定、可重复跑到全绿。
const tests = [
